...
if (CLIENT) then
	...
	function PLUGIN.Mute(PLAYER)
		...
		RunConsoleCommand("ASS_Mute", PLAYER:UniqueID())
	end
	
	function PLUGIN.UnMute(PLAYER)
		if (!PLAYER:IsValid()) then return end

		RunConsoleCommand("ASS_Unmute", PLAYER:UniqueID())
	end
	
	function PLUGIN.Gag(PLAYER)
		if (!PLAYER:IsValid()) then return end
		
		RunConsoleCommand("ASS_Gag", PLAYER:UniqueID())
	end
	
	function PLUGIN.UnGag(PLAYER)
		if (!PLAYER:IsValid()) then return end

		RunConsoleCommand("ASS_Ungag", PLAYER:UniqueID())
	end
