...
if (SERVER) then
	...
	function PLUGIN.PlayerInitialSpawn(pl)
		...
		if PlayerEntPending[pl:UniqueID()] then
			local cleanupents = cleanup.GetList()[pl:UniqueID()]
			PlayerEntPending[pl:UniqueID()] = nil
	...
	function PLUGIN.ShowBuddyList( CP )
		...
		for k,v in pairs(CurPlayers) do
			...
			if (v != LocalPlayer()) then
				...
				CP:AddControl("Button", { Text = "Add", Label = v:Nick() .. " (" .. v:AssId() .. ")", Command = "ASS_PropProtectAddBuddy_CL " .. v:UniqueID() } )
