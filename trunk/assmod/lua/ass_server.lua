...
local ChatLogFilter = { ASS_ACL_SPEECH, ASS_ACL_KILL_SILENT } -- ASS_ACL_JOIN_QUIT was once in here
...
function ASS_PlayerInitialSpawn( PLAYER )
	...
	if (ASS_IsLan()) then 
		...
		PLAYER:SetNetworkedString("ASS_AssID", PLAYER:SteamID())
	end 
...
function ASS_FullNick( PLAYER )		return "\"" .. PLAYER:Nick() .. "\" (" .. PLAYER:SteamID() .. " | " .. LevelToString(PLAYER:GetLevel()) .. ")"		end
...
concommand.Add( "ASS_UnbanList",
		function (pl, cmd, args)
			...
			ASS_BeginProgress("ASS_BannedPlayer", "Recieving banned list...", n, 0)
			for id, entry in pairs(PlayerRankings) do
				if (entry.Rank == ASS_LVL_BANNED) then
...
concommand.Add("ASS_SetClientTell",function(PLAYER, CMD, ARGS)
		if (PLAYER:HasLevel(ASS_LVL_SERVER_OWNER)) then
